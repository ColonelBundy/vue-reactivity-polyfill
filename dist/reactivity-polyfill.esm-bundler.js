...
import { toRaw, track, ITERATE_KEY, isReadonly, isProxy, reactive, isReactive, trigger, readonly, pauseTracking, enableTracking } from '@vue/reactivity';
...
import { isObject as isObject$1, hasOwn, isArray, isString } from '@vue/shared';
...
originFrom && (Array.from = function (arrayLike) {
  ...
  var raw = toRaw(arrayLike); // at least call once for ITERATE_KEY

  track(raw, 'interate', ITERATE_KEY); // check forEach
  // @ts-ignore
...
function handleReadonly(proxy, key) {
  ...
  var _isReadonly = isReadonly(proxy);
...
function addProp(proxy, key, val) {
  var _isReactive = isReactive(proxy);

  if (_isReactive) {
    var target = toRaw(proxy);
...
methodsToPatch.forEach(function (method) {
  ...
  def(arrayMethods, method, {
    ...
    value: function mutator() {
      ...
      var proxy = isProxy(this) ? this : target[ReactiveFlags.REACTIVE] || target[ReactiveFlags.READONLY];
      ...
      while (i < newLen || j < oldLen) {
        if (i < newLen) {
          ...
          if (!(i in oldThis)) {
            ...
            if (oldVal !== newVal) {
              trigger(target, 'set', i, newVal, oldVal);
            }
          }
        } else {
          delete proxy[i];
          ...
          trigger(target, 'delete', i, undefined, oldThis[i]);
        }
      ...
      trigger(target, 'clear', 'length', target.length, oldLen);
      return result;
    }
  });
});
...
def(arrayMethods, method, {
  ...
  value: function mutator(callback, thisArg) {
    ...
    var _isReadonly = isReadonly(this);

    var _isReactive = isReactive(this);

    var proxy = _isReadonly || _isReactive ? this : target[ReactiveFlags.REACTIVE] || target[ReactiveFlags.READONLY];
    !_isReadonly && track(target, 'iterate', ITERATE_KEY);
...
function del(o, key) {
  ...
  delete target[key];
  delete proxy[key];
  trigger(target, 'delete', key, undefined, oldVal);
}

export { del, get, getLength, set };
